...
fn bench_random_component_1(group: &mut BenchmarkGroup<'_, WallTime>, component_count: usize) {
    group.bench_function(
        format!("random_{component_count}_components_1_term/bevy"),
        |bencher| {
            let (mut world, masks) =
            ...
            let mut query = world.query::<(BevyEntity, &A)>();
            assert_eq!(query.iter(&world).count(), expected);
            bencher.iter(|| {
                let mut checksum = 0_u64;
                query.iter(&world).for_each(|(entity, a)| {
                    checksum =
                        add_random_fragment_component_1_checksum(checksum, entity.to_bits(), a.0);
                });
                checksum
            });
        },
...
fn bench_random_component_4(group: &mut BenchmarkGroup<'_, WallTime>, component_count: usize) {
    group.bench_function(
        format!("random_{component_count}_components_4_terms/bevy"),
        |bencher| {
            let (mut world, masks) =
                random_fragmented_component_world(component_count, RANDOM_FRAGMENT_ENTITY_COUNT);
            let expected = random_fragment_match_count(&masks, 4);
            let mut query = world.query::<(BevyEntity, &A, &B, &C, &D)>();
            assert_eq!(query.iter(&world).count(), expected);
            bencher.iter(|| {
                let mut checksum = 0_u64;
                query.iter(&world).for_each(|(entity, a, b, c, d)| {
...
fn bench_random_component_8(group: &mut BenchmarkGroup<'_, WallTime>, component_count: usize) {
    group.bench_function(
        format!("random_{component_count}_components_8_terms/bevy"),
        |bencher| {
            let (mut world, masks) =
                random_fragmented_component_world(component_count, RANDOM_FRAGMENT_ENTITY_COUNT);
            let expected = random_fragment_match_count(&masks, 8);
            let mut query = world.query::<(BevyEntity, &A, &B, &C, &D, &E, &F, &G, &H)>();
            assert_eq!(query.iter(&world).count(), expected);
            bencher.iter(|| {
                let mut checksum = 0_u64;
                query
...
macro_rules! bench_random_tags {
    ($name:ident, $terms:literal, $query:ty, $pattern:pat_param, $entity:ident) => {
        fn $name(group: &mut BenchmarkGroup<'_, WallTime>, component_count: usize) {
            ...
            group.bench_function(
                format!("random_{component_count}_tags_{}_{suffix}/bevy", $terms),
                |bencher| {
                    let (mut world, masks) =
                    ...
                    bencher.iter(|| {
                        let mut checksum = 0_u64;
                        query.iter(&world).for_each(|$pattern| {
                            checksum = checksum.wrapping_add($entity.to_bits());
                        });
                        checksum
                    });
                },
...
bench_random_tags!(
    bench_random_tag_4,
    4,
    (BevyEntity, &TagA, &TagB, &TagC, &TagD),
    (entity, _, _, _, _),
    entity
);
bench_random_tags!(
    bench_random_tag_8,
    8,
    (BevyEntity, &TagA, &TagB, &TagC, &TagD, &TagE, &TagF, &TagG, &TagH),
