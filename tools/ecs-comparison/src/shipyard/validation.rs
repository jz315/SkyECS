...
use crate::common::{
    ...
    expected_random_fragment_checksum, light_bundle, random_fragment_match_count, DataComponent,
    Health, PositionComponent, RotationComponent, TagA, TagB, TagC, TagD, TagE, TagF, TagG, TagH,
    TagI, TagJ, TagK, TagL, TagM, TagN, TagO, TagP, TransformComponent, VelocityComponent, A, B, C,
...
fn validate_random_access() {
    ...
    let random_entities: Vec<_> = (0..CONTRACT_ENTITY_COUNT)
        .map(|index| random_world.add_entity(random_access_bundle(index)))
        .collect();
